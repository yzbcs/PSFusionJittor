...
class FReLU(nn.Module):
    ...
    def execute(self, x):
        ...
        x1 = self.bn_frelu(x1)
        # 使用element-wise maximum而不是stack+max来保持张量维度
        out = jt.maximum(x, x1)
        return out
...
class GEFM(nn.Module):
    def __init__(self, in_C, out_C):
        ...
        self.gamma1 = nn.Parameter(jt.zeros(1))
        self.gamma2 = nn.Parameter(jt.zeros(1))
        self.softmax = nn.Softmax(dim=-1)
        
    def execute(self, x, y):
        Q = self.Q(jt.concat([x,y], dim=1))
        RGB_K = self.RGB_K(x)
        RGB_V = self.RGB_V(x)
        ...
        RGB_mask = jt.bmm(RGB_K, RGB_Q)
        RGB_mask = self.softmax(RGB_mask)
        RGB_refine = jt.bmm(RGB_V, RGB_mask.permute(0, 2, 1))
        ...
        INF_mask = jt.bmm(INF_K, INF_Q)
        INF_mask = self.softmax(INF_mask)
        INF_refine = jt.bmm(INF_V, INF_mask.permute(0, 2, 1))
        INF_refine = INF_refine.view(m_batchsize, -1, height,width) 
        INF_refine = self.gamma2 * INF_refine + x
        
        out = self.Second_reduce(jt.concat([RGB_refine, INF_refine], dim=1))        
        return out  
...
class DenseLayer(nn.Module):
    ...
    def execute(self, in_feat):
        ...
        for i in self.denseblock:
            feats = i(jt.concat((*out_feats, down_feats), dim=1))
            out_feats.append(feats)
        feats = jt.concat((in_feat, feats), dim=1)
...
class DSRM(nn.Module):
    ...
    def execute(self, x):
        x1 = self.block1(x)
        x2 = self.block2(jt.concat([x, x1], dim=1))
        x3 = self.block3(jt.concat([x, x1, x2], dim=1))
        out = self.block4(jt.concat([x, x1, x2, x3], dim=1))
        return out
...
class S2P2(nn.Module):
    ...
    def execute(self, feat):
        ...
        weight = jt.exp(binary)
        weight = weight[:, 1:2, :, :] / jt.sum(weight, dim=1, keepdim=True)
        ...
        feat_boundary = jt.concat([feat_sematic, self.up2x(feat)], dim=1)
...
class PSF(nn.Module):
    ...
    def execute(self, rgb, depth):
        ...
        # encoder
        rgb1, rgb2 = self.encoder_rgb_conv1(rgb)
        rgb2 = self.encoder_rgb_bn1(rgb2)
        rgb2 = self.encoder_rgb_relu(rgb2)

        thermal1, thermal2 = self.encoder_thermal_conv1(thermal)
        thermal2 = self.encoder_thermal_bn1(thermal2)
        thermal2 = self.encoder_thermal_relu(thermal2)

        rgb3 = self.encoder_rgb_maxpool(rgb2)
        thermal3 = self.encoder_thermal_maxpool(thermal2)
        rgb3 = self.encoder_rgb_layer3(rgb3)
        thermal3 = self.encoder_thermal_layer3(thermal3)

        rgb4= self.encoder_rgb_layer4(rgb3)
        thermal4 = self.encoder_thermal_layer4(thermal3)

        rgb5 = self.encoder_rgb_layer5(rgb4)
        thermal5 = self.encoder_thermal_layer5(thermal4)

        rgb6 = self.encoder_rgb_layer6(rgb5)
        thermal6 = self.encoder_thermal_layer6(thermal5)
        ...
        seg_f = jt.concat(seg_fused_f, dim=1)
